    ...
    try {
      if (process.env.ZOHO_EMAIL && process.env.ZOHO_PASSWORD) {
        ...
        const emailHtml = await render(
          WelcomeEmail({ email, position: position || 0 }),
        );
        ...
        console.error(
          "CRITICAL: ZOHO_EMAIL or ZOHO_PASSWORD not found in environment variables",
        );
        console.error(
          "Available env vars:",
          Object.keys(process.env).filter((k) => k.includes("ZOHO")),
        );
      }
    } catch (emailError: unknown) {
      const error = emailError as Error & { code?: string; response?: unknown };
      ...
      if (error?.code === "EAUTH") {
        console.error(
          "Zoho Auth Error: Check email/password or enable app-specific password",
        );
      }
